...
static const char* position_names[] = {
    "FIRST", "SECOND", "THIRD", "SHORT", "LEFT", "CENTER", "RIGHT", "CATCHER", "PITCHER"
};
    ...
    while (players_selected < TEAM_SIZE) {
        ...
        *(end + 1) = '\0';
        ...
        printf("        %-2s %4d %-18s .%03d  %c\n",
            position_names[player->position / 10],
            player->year,
            player->team,
            player->batting_avg,
            player->hand);
...
void play_game(GameState* game) {
    while (game->inning <= MAX_INNINGS || game->visiting_score == game->home_score) {
        ...
        if (game->inning > MAX_INNINGS && game->home_score != game->visiting_score) {
            break;
        }
    }
}
...
void print_final_score(GameState* game) {
    ...
    printf("VISITORS        %02d  %02d  %02d\n",
        game->visiting_score, game->visiting_hits, game->visiting_errors);
    printf("HOMETEAM        %02d  %02d  %02d\n",
        game->home_score, game->home_hits, game->home_errors);
    printf("\n\n");
}
